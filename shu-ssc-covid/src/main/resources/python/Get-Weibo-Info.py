...
class data_spider:
    def __init__(self):
        ...
        self.since_date = datetime(2021, 12, 20, 0, 0, 0).strftime('%Y-%m-%d')
    ...
    def get_weibo_json(self, page):
        """获取网页中微博json数据"""
        params = {'containerid': '107603' + str(weibo_config['user_id']), 'page': page}
        js = self.get_json(params)
        return js
    ...
    def weibo_to_mysql(self, wrote_count):
        ...
        for w in info_list:
            ...
            weibo_list.append(w)
        # 在'weibo'表中插入或更新微博数据
        self.mysql_insert(mysql_config, 'weibo_cctv_News', retweet_list)
        self.mysql_insert(mysql_config, 'weibo_cctv_News', weibo_list)
...
if __name__ == "__main__":
    import pymysql
    mysql_config = weibo_config["mysql_config"]
    connection = pymysql.connect(host=mysql_config["host"], user=mysql_config["user"], password=mysql_config["password"], charset=mysql_config["charset"])
    cursor = connection.cursor()
    sql = "CREATE DATABASE IF NOT EXISTS %s;" % mysql_config["db"]
    cursor.execute(sql)
    cursor.close()
    connection.close()
    connection = pymysql.connect(host=mysql_config["host"], user=mysql_config["user"], password=mysql_config["password"], database=mysql_config["db"], charset=mysql_config["charset"])
    cursor = connection.cursor()
    sql = "DROP TABLE IF EXISTS weibo_cctv_News;"
    cursor.execute(sql)
    sql = """
CREATE TABLE weibo_cctv_News (
    ...
    `created_at` TEXT NOT NULL COMMENT '发布时间',
